...
class MediaScanner:
        ...
        if recursive:
            try:
                for root, dirs, files in os.walk(str(directory), topdown=True, onerror=None, followlinks=False):
                    ...
                    # Only remove directories that are clearly inaccessible
    ...
    def analyze_media(self, media_info: MediaInfo) -> MediaInfo:
        ...
        try:
            ...
            # CRITICAL optimizations for network volumes:
            # 1. Use -show_entries to request ONLY the fields we need (avoids full file scan)
            # 2. Aggressive probesize/analyzeduration limits
            # 3. -count_packets:v:0 0 to avoid counting all packets
            cmd = [
                'ffprobe',
                '-v', 'error',
                # '-select_streams', 'v:0',
                '-probesize', '1000000',  # 1MB - enough for headers
            ...
            if video_stream:
                ...
                if 'bit_rate' in video_stream and video_stream['bit_rate']:
                    try:
                        media_info.bitrate = int(video_stream['bit_rate']) // 1000  # Convert to kbps
                    except (ValueError, TypeError):
                        pass
                elif 'bit_rate' in data.get('format', {}) and data['format']['bit_rate']:
                    try:
                        media_info.bitrate = int(data['format']['bit_rate']) // 1000
                    except (ValueError, TypeError):
                        pass
                
                # If no bitrate from ffprobe, estimate from file size and duration
                if media_info.bitrate == 0 and media_info.file_size > 0:
                    duration = float(data.get('format', {}).get('duration', 0))
                    if duration > 0:
                        media_info.duration = duration
                        # Estimate: (file_bytes * 8) / duration_seconds / 1000 = kbps
