...
class BatchEncoder(QObject):
    ...
    def _emit_log(self, log_type: str, message: str, color: str):
        """Emit log signal and callback."""
        self.log_signal.emit(log_type, message, color)
        if self.on_log:
            self.on_log(log_type, message, color)

    def _emit_progress(self, job_index: int, progress: float, status: str, speed: float, eta: str):
        """Emit progress signal and callback."""
        self.progress_signal.emit(job_index, progress, status, speed, eta)
        if self.on_progress:
            self.on_progress(job_index, progress, status, speed, eta)

    def _emit_job_complete(self, job_index: int, success: bool, message: str):
        """Emit job complete signal and callback."""
    ...
    def start_encoding(self):
        ...
        self.is_running = False
        # Only emit all_complete if we finished naturally, not if stopped
        if not self.should_stop:
            self.all_complete.emit()
        else:
            self._emit_log("info", "Encoding stopped by user", "inherit")
    ...
    def _encode_job(self, job_index: int, job: EncodingJob):
        ...
        try:
            ...
            # Log file start
            self._emit_log("file_start", f"{str(job.media_info.path)}|{str(job.output_path)}", "")
            ...
            try:
                ...
                for line in iter(process.stderr.readline, ''):
                    ...
                    if any(err in line_lower for err in ['error', 'failed', 'invalid', 'cannot', 'unable', 'not found', 'no such', 'warning']):
                        ...
                        self._emit_log("ffmpeg_error", line.strip(), "#ff6b6b")
                    ...
                    if "frame=" in line and "time=" in line:
                        try:
                            ...
                            # Calculate ETA using frame-based or time-based method
                            eta = "--:--"

                            # Try frame-based calculation first
                            ...
                            # Fallback to time-based calculation if frame info not available
                            elif duration > 0 and current_time > 0 and speed > 0:
                                remaining_seconds = (duration - current_time) / speed
                                eta_minutes = int(remaining_seconds // 60)
                                eta_seconds = int(remaining_seconds % 60)
                                eta = f"{eta_minutes:02d}:{eta_seconds:02d}"
            ...
            if process.returncode == 0:
                ...
                if job.output_path.exists() and job.output_path.stat().st_size > 0:
                    ...
                    self._emit_job_complete(job_index, False, job.error_message)
            else:
                job.status = "failed"
                ...
                self._emit_log("error", job.error_message, "#ff6b6b")
                self._emit_job_complete(job_index, False, job.error_message)

        except Exception as e:
            job.status = "failed"
            job.error_message = str(e)
            self._emit_job_complete(job_index, False, str(e))
    ...
    def _build_ffmpeg_command(self, media_info: MediaInfo, output_path: Path) -> List[str]:
        ...
        if skip_video:
            ...
            # Get codec selection based on use_gpu flag
            use_gpu = self.encoding_params.get("use_gpu", False)

            # If GPU is enabled, use NVENC codec; otherwise use software codec
            if use_gpu:
            ...
            try:
                if 'nvenc' in codec.lower():
                    ...
                    if codec.lower() not in enc_list.stdout.lower():
                        ...
                        fallback = 'libx265'
                        self._emit_log('warning', f"Requested GPU encoder '{codec}' not available; falling back to {fallback}", '#ffa500')
                        codec = fallback
                        use_gpu = False
            except Exception:
    ...
    @staticmethod
    def cleanup_encoded_folders(media_path: Path) -> tuple[int, int, str]:
        """
        Remove all /encoded subdirectories from media path.

        Args:
            media_path: Root path to scan for /encoded folders

        Returns:
            Tuple of (folders_removed, files_removed, status_message)
        """
        import shutil
        ...
        if not media_path.exists():
            return 0, 0, f"Media path does not exist: {media_path}"

        folders_removed = 0
        files_removed = 0
        errors = []

        # Find all /encoded directories
        encoded_dirs = list(media_path.glob("**/encoded"))

        if not encoded_dirs:
            return 0, 0, "No /encoded folders found"

        for encoded_dir in encoded_dirs:
            try:
                # Count files before removal
                file_count = sum(1 for _ in encoded_dir.rglob("*") if _.is_file())
        ...
        if errors:
            status += f" (with {len(errors)} error(s))"

        return folders_removed, files_removed, status
